...
function changeStatus(id, newStatus) {
    tasks = tasks.map(t => t.id === id ? { ...t, status: newStatus, completed: newStatus === 'done' } : t);
    renderTasks();
    saveTasks();
}
...
function updateStats() {
    ...
    const overdue   = tasks.filter(t => {
        if (t.status === 'archived' || t.status === 'done' || t.status === 'cancelled') return false;
    ...
    // Linear progress bar
    const linearBar = document.getElementById('progressLinearBar');
...
function updateDonut(todo, inProgress, done, cancelled, total) {
    const circ = 2 * Math.PI * 80; // r=80 → ~503
    const gap  = 3;
    const segs = [
    ...
    if (track) {
        ...
        track.style.stroke = borderColor || 'rgba(191,132,64,0.18)';
    }
